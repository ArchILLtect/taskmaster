...
const TaskItem = ({ task, onTaskDeleted, onHighlight, isHighlighted }) => {
    ...
    return (
        <div
            className={`flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-black shadow-md p-2 rounded-md ${
                isHighlighted ? 'bg-red-100 dark:bg-red-900' : ''
            }`}
        >
            <div className="flex flex-col sm:flex-row gap-3 w-full">
                <span className="w-full sm:w-48 dark:text-gray-200 whitespace-nowrap overflow-hidden text-ellipsis">{task.taskName}</span>
                <p className="w-full dark:text-gray-200">{task.taskDescription}</p>
            </div>
            <div className="flex gap-3 dark:text-gray-200">
                <span className="w-20 text-center">{task.taskStatus}</span>
                <span className="w-20 text-center">{task.taskPriority}</span>
                <button
                    onClick={() => {
                        setShowDialog(true);
                        onHighlight(task.taskID); // Notify App.js of the task to highlight
                    }}
                    className="bg-red-500 font-mono text-white dark:font-bold dark:text-black px-3 py-1 rounded-md hover:bg-red-600 transition duration-200"
                >
                    Delete
                </button>
            </div>
