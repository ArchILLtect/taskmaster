...
export const TaskRow = ({ task, list, to, showLists, onToggleComplete, onDelete }: TaskRowProps) => {
  ...
  const isInSystemInbox = task.listId === (inboxListId ?? "");
  ...
  const renderRow = (isActive: boolean) => (
    <Box
      ...
      cursor={isInSystemInbox ? "default" : "pointer"}
      _hover={{ bg: isInSystemInbox ? "gray.50" : "blue.100" }}
    >
          ...
          {showLists ? (
            <Flex w="150px" flexDirection={"column"} gap={3} minW="0">
              <Badge fontSize="sm" color="gray.500">
                <Text truncate>{list.name || "Unknown List"}</Text>
              </Badge>
              <Badge fontSize="sm" color="gray.500" justifyContent={"space-between"}>
                <Text fontWeight={800} color={isOverdue ? "red.400" : "gray.800"}>
                  Due:{" "}
                </Text>
                {isOverdue && <Icon as={FcHighPriority} mr={1} />}
                <Text truncate>{formatDueDate(task.dueAt, { noneLabel: "Anytime" })}</Text>
              </Badge>
            </Flex>
          ) : null}
  ...
  // Inbox tasks are a staging area; avoid navigating into the list/task pane stack.
  if (isInSystemInbox) return renderRow(false);

  return <RouterLink to={to}>{({ isActive }) => renderRow(isActive)}</RouterLink>;
}