...
export const inboxService = {
  ...
  setDueSoonWindowDays(days: number) {
    ...
    setState({ ...s, dueSoonWindowDays: normalized });
  },

  dismiss(taskId: string) {
    const s = getState();
    if (s.dismissedTaskIds.includes(taskId)) return;
    setState({ ...s, dismissedTaskIds: [...s.dismissedTaskIds, taskId] });
  },

  undismiss(taskId: string) {
    const s = getState();
    setState({ ...s, dismissedTaskIds: s.dismissedTaskIds.filter((id) => id !== taskId) });
  },

  markViewedNow() {
    const s = getState();
    setState({ ...s, lastViewedAt: isoNow() });
  },
